...
namespace HyperEdge.Sdk.Unity.Flexi
{
    ...
    internal class ConversionHandler : IConversionHandler
    {
        private struct ConversionQuery : IEquatable<ConversionQuery>
        {
            ...
            {
                return (fromType, toType).GetHashCode();
            }
        }

        private readonly Dictionary<ConversionQuery, object> converterTable = new Dictionary<ConversionQuery, object>();
        private readonly Dictionary<ConversionQuery, Func<object, object>> converterBoxedTable = new Dictionary<ConversionQuery, Func<object, object>>();

        public void Handle<TFrom, TTo>(Func<TFrom, TTo> converter)
        {
            var query = new ConversionQuery(typeof(TFrom), typeof(TTo));
            if (converterTable.ContainsKey(query))
            {
                Logger.Error($"[{nameof(ConversionHandler)}] Converter has already exists! ({typeof(TFrom).Name} => {typeof(TTo).Name})");
                return;
            }

            converterTable.Add(query, converter);
