...
using HyperEdge.Sdk.Unity;


namespace HyperEdge.Sdk.Unity.Flexi.GraphViewEditor
{
    using NodeData = HyperEdge.Sdk.Unity.Flexi.Node;
    using PortData = HyperEdge.Sdk.Unity.Flexi.Port;
    using EdgeData = HyperEdge.Sdk.Unity.Flexi.Edge;
    ...
    public class AbilityGraphEditorWindow : EditorWindow
    {
        ...
        private bool SaveToHyperEdgeProject(GraphAsset asset)
        {
            var appData = AppDataManager.Default.CurrentAppData;
            var prjPath = HyperEdgePy.GetPythonScriptsPath(appData.Name);
            var abilityAsset = asset as AbilityAsset;
            if (abilityAsset is null)
            {
                return false;
            }
            var fname = $"{prjPath}/data/NodeFlows/{abilityAsset.Data.name}.json";
            string json = JsonConvert.SerializeObject(abilityAsset.Data);
        ...
        private bool SaveFile()
        {
            ...
            if (isBlankAssets)
            {
                ...
                SetDirty(false);
                return SaveToHyperEdgeProject(tempAsset);
            }
            ...
            SetDirty(false);
            return SaveToHyperEdgeProject(currentAsset);
        }
        ...
        {
	        var abilityAsset = tempAsset as AbilityAsset;
            if (abilityAsset is null)
            {
                return;
            }
