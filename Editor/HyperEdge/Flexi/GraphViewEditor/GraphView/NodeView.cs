...
namespace HyperEdge.Sdk.Unity.Flexi.GraphViewEditor
{
    ...
    public class NodeView : UnityEditor.Experimental.GraphView.Node
    {
        ...
        private static readonly Color MISSING_PORT_COLOR = new Color(1f, 0f, 0f);

        private VisualElement inputFieldContainer;
        private readonly List<PortView> inputPortViews = new List<PortView>();
        private readonly List<PortView> outputPortViews = new List<PortView>();
        private readonly Dictionary<PortView, VisualElement> inputPortToFieldTable = new Dictionary<PortView, VisualElement>();
        ...
        private readonly Dictionary<PortData, Port> portDataToViewTable = new Dictionary<PortData, Port>();
        private readonly Dictionary<Port, PortData> portViewToDataTable = new Dictionary<Port, PortData>();
        ...
        public NodeView(NodeData nodeData, AbilityGraphEditorWindow window, AbilityGraphView graphView)
            : base(EditorConst.PackagePath + "GraphViewEditor/UiAssets/Node.uxml")
        {
            UseDefaultStyling();
        ...
        {
            if (!(nodeData is SubgraphNode || nodeData is GraphInputNode || nodeData is GraphOutputNode))
            {
	    ...
	    if (node is EntryNode)
	    {
            ...
            else if (node is SubgraphNode || node is GraphInputNode || node is GraphOutputNode)
            {
        ...
        private static string GetPortPrettyName(string portName)
        {
            ...
            {
                return portName.Substring(0, portName.Length - 4);
            }
