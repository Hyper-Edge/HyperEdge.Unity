...
namespace HyperEdge.Sdk.Unity.Flexi.GraphViewEditor
{
    internal static class SearchTreeEntriesCache
    {
        ...
        private static readonly HashSet<string> TEST_ASSEMBLY_NAMES = new HashSet<string> {
            "HyperEdge.Sdk.Unity.Flexi.Editor.Tests",
            "HyperEdge.Sdk.Unity.Flexi.PerformanceTests",
        };
#endif
        ...
        private static readonly Dictionary<Color, Texture2D> customNodeMenuIcons = new Dictionary<Color, Texture2D>();

        private static readonly List<SearchTreeEntry> searchTreeEntries = new List<SearchTreeEntry>();
        private static readonly List<SearchTreeEntry> nodeTreeEntries = new List<SearchTreeEntry>();
        private static readonly List<SearchTreeEntry> macroTreeEntries = new List<SearchTreeEntry>();
        ...
        private static Dictionary<string, string> GetMacroKeyToNameTable()
        {
            ...
            for (var i = 0; i < guids.Length; i++)
            {
                ...
                string name = assetPaths[i].Split('/')[assetPaths.Length - 1];
